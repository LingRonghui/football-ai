...
class SVMExplainerDialog(ExplainerDialog):
    ...
    def __init__(self, df: pd.DataFrame, model_db: ModelDatabase):
        ...
        super().__init__(df=df, model_db=model_db, title='Support Vector Machine (SVM) 解释器', width=400, height=620)
    ...
    def _add_additional_widgets(self, root: QVBoxLayout):
        ...
        coeff_label_row.addWidget(QLabel('系数条形图'))
        ...
        self._coeff_btn = QPushButton('绘制系数')
        ...
        vis_label_row.addWidget(QLabel('模型可视化图'))
        ...
        features_row.addWidget(QLabel('x-特征： '))
        ...
        features_row.addWidget(QLabel(' y-特征： '))
        ...
        self._visualize_btn = QPushButton('可视化模型')
    ...
    def _plot_coeffs(self):
        ...
        PlotWindow(ax=ax, parent=self, title='系数分析').show()
    ...
    def _plot_model(self):
        ...
        PlotWindow(ax=ax, parent=self, title='模型可视化').show()
