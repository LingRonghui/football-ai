...
class NeuralNetworkExplainerDialog(ExplainerDialog):
    ...
    def __init__(self, df: pd.DataFrame, model_db: ModelDatabase):
        ...
        super().__init__(df=df, model_db=model_db, title='Neural Network 解释器', width=400, height=600)
    ...
    def _add_additional_widgets(self, root: QVBoxLayout):
        ...
        importance_label_row.addWidget(QLabel('特征注意力条形图'))
        ...
        self._importance_btn = QPushButton('绘制特征注意力')
        ...
        self._importance_btn.setToolTip('绘制各特征的注意力。仅当 VSN 为 True 时支持。')
    ...
    def _plot_importance(self):
        ...
        PlotWindow(ax=ax, parent=self, title='特征注意力分析').show()
