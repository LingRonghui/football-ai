...
class RandomForestExplainerDialog(ExplainerDialog):
    ...
    def _add_additional_widgets(self, root: QVBoxLayout):
        ...
        importance_label_row.addWidget(QLabel('重要性条形图'))
        ...
        self._importance_btn = QPushButton('绘制特征不纯度')
        ...
        tree_label_row.addWidget(QLabel('树规则图'))
        ...
        self._rules_btn = QPushButton('可视化模型')
    ...
    def _plot_importance(self):
        ...
        PlotWindow(ax=ax, parent=self, title='特征不纯度分析').show()

    def _plot_tree(self):
        max_depth = self._spin_depth.value()
        ax = self._explainer.plot_tree_rules(max_depth=max_depth)
        PlotWindow(ax=ax, parent=self, title='树可视化').show()
