...
class KNNExplainerDialog(ExplainerDialog):
    ...
    def __init__(self, df: pd.DataFrame, model_db: ModelDatabase):
        ...
        super().__init__(df=df, model_db=model_db, title='K-Nearest Neighbors (KNN) 解释器', width=400, height=600)
    ...
    def _add_additional_widgets(self, root: QVBoxLayout):
        ...
        features_row.addWidget(QLabel(' 比赛索引： '))
    ...
    def _set_tooltip(self):
        ...
        tooltip = f'{row["Date"]} - {row["Home"]} 对阵 {row["Away"]} - {row["HG"]}-{row["AG"]}: {row["Result"]}'
