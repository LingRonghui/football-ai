...
class ExplainerDialog(QDialog):
    ...
    def _add_widgets(self):
        ...
        boundary_label_row.addWidget(QLabel('决策边界图'))
        ...
        boundary_row.addWidget(QLabel('x 特征: '))
        ...
        boundary_row.addWidget(QLabel(' y 特征: '))
        ...
        self._boundary_btn = QPushButton('绘制决策边界')
        ...
        pdp_label_row.addWidget(QLabel('部分依赖图 (PDP)'))
        ...
        self._pdp_btn = QPushButton('绘制部分依赖图')
        ...
        waterfall_label_row.addWidget(QLabel('瀑布图'))
        ...
        waterfall_row.addWidget(QLabel('比赛索引: '))
        ...
        waterfall_row.addWidget(QLabel(' 预测目标: '))
        ...
        self._waterfall_btn = QPushButton('绘制瀑布图')
        ...
        shap_label_row.addWidget(QLabel('Shap 条形图'))
        ...
        shap_row.addWidget(QLabel('预测目标: '))
        shap_row.addWidget(self._combo_shap_targets)

        self._check_shap_clusters = QCheckBox('应用聚类')
        ...
        self._shap_btn = QPushButton('绘制 Shap 值')
    ...
    def _plot_boundaries(self):
        ...
        if feat1 == '' or feat2 == '':
            QMessageBox.critical(self, '错误', '未选择特征。')
            return

        ax = self._explainer.boundary_plot([feat1, feat2])
        PlotWindow(ax=ax, parent=self, title='决策边界分析').show()
    ...
    def _plot_pdp(self):
        ...
        if feature == '' or target == '':
            QMessageBox.critical(self, '错误', '未选择特征或预测目标。')
            return

        ax = self._explainer.partial_dependence_plot(feature=feature, target=target)
        PlotWindow(ax=ax, parent=self, title='部分依赖分析').show()
    ...
    def _plot_waterfall(self):
        ...
        if target == '':
            QMessageBox.critical(self, '错误', '未选择预测目标。')
            return

        ax = self._explainer.instance_waterfall_plot(match_index=match_index, target=target)
        PlotWindow(ax=ax, parent=self, title='瀑布图分析').show()
    ...
    def _plot_shap(self):
        ...
        PlotWindow(ax=ax, parent=self, title='Shap 分析').show()
