...
class ModelManagerDialogWindow(QDialog):
    ...
    def _view_metrics(self):
        ...
        if 'tune' in model_results:
            SimpleTableDialog(df=model_results['tune'], parent=self, title='滑动交叉验证结果').show()
        if 'cv' in model_results:
            SimpleTableDialog(df=model_results['cv'], parent=self, title='交叉验证结果').show()
        if 'sliding-cv' in model_results:
            SimpleTableDialog(df=model_results['sliding-cv'], parent=self, title='滑动交叉验证结果').show()
        SimpleTableDialog(df=model_results['fit'], parent=self, title='训练结果').show()
