...
class EvaluatorDialog(QDialog):
    ...
    def __init__(self, df: pd.DataFrame, model_db: ModelDatabase):
        ...
        self._dataset_mask_dict = {'全部': np.array([True]*self._df.shape[0], dtype=bool), '训练集': None, '验证集': None}
        self._y_true_dict = {
    ...
    def _add_widgets(self):
        ...
        model_hbox.addWidget(QLabel(' 数据集: '))
        ...
        filters_hbox.addWidget(QLabel(' 赔率区间: '))
        ...
        self._slider_percentile_1 = make_percentile_slider(slider_row=result_hbox, name='主队')
        self._slider_percentile_X = make_percentile_slider(slider_row=result_hbox, name='平局')
        self._slider_percentile_2 = make_percentile_slider(slider_row=result_hbox, name='客队')
        result_hbox.addStretch(1)
        root.addLayout(result_hbox)

        uo_hbox = QHBoxLayout()
        uo_hbox.addStretch(1)
        self._slider_percentile_under = make_percentile_slider(slider_row=uo_hbox, name='小')
        self._slider_percentile_over = make_percentile_slider(slider_row=uo_hbox, name='大')
        uo_hbox.addStretch(1)
        ...
        self._store_filter_btn = QPushButton('存储过滤器')
        ...
        self._delete_filter_btn = QPushButton('删除过滤器')
        ...
        self._seasonal_metrics_btn = QPushButton('赛季指标')
        ...
        self._acc_label = QLabel('准确率: 0.0')
        self._acc_label.setFont(font)
        self._f1_label = QLabel('F1:  0.0')
        self._f1_label.setFont(font)
        self._prec_label = QLabel('精确率:  0.0')
        self._prec_label.setFont(font)
        self._rec_label = QLabel('召回率:  0.0')
        self._rec_label.setFont(font)
        self._samples_label = QLabel(f'样本数: {self._df.shape[0]}')
        self._samples_label.setFont(font)
        self._profit_balance_label = QLabel('盈利平衡: 0.0')
    ...
    def _reset_evaluator_state(self):
        ...
        self._dataset_mask_dict.update({'训练集': None, '验证集': None})
    ...
    def _load_model(self):
        ...
        self._dataset_mask_dict.update({
            '训练集': np.array([False]*self._num_eval_samples + [True]*self._num_train_samples, dtype=bool),
            '验证集': np.array([True]*self._num_eval_samples + [False]*self._num_train_samples, dtype=bool),
        })
    ...
    def _update_table_and_metrics(self):
        ...
        def _update():
            ...
            if self._y_pred is not None:
                ...
                self._acc_label.setText(f'准确率: {metrics_df.at[0, "Accuracy"]}')
                self._f1_label.setText(f'F1: {metrics_df.at[0, "F1"]}')
                self._prec_label.setText(f'精确率: {metrics_df.at[0, "Precision"]}')
                self._rec_label.setText(f'召回率: {metrics_df.at[0, "Recall"]}')
                self._samples_label.setText(f'正确: {len(highlight_ids)}/{y_pred.shape[0]}')
                self._profit_balance_label.setText(f'盈利平衡: {self._compute_profit_balance(y_pred=y_pred, filter_mask=filter_mask)}')
            else:
                self._acc_label.setText('准确率: 0.0')
                self._f1_label.setText('F1: 0.0')
                self._prec_label.setText('精确率: 0.0')
                self._rec_label.setText('召回率: 0.0')
                self._samples_label.setText(f'样本数: {self._df.shape[0]}')
                self._profit_balance_label.setText('盈利平衡: 0.0')

        TaskRunnerDialog(title='正在更新表格', info='正在评估比赛...', parent=self, task_fn=_update).run()
    ...
    def _display_seasonal_metrics(self):
        ...
        SimpleTableDialog(df=seasonal_metrics_df, parent=self, title='赛季指标').show()
    ...
    def _set_percentile_tooltips(self):
        ...
        if self._prob_percentiles is None:
            ...
            tooltip_x = '概率(平局): 0.0'
            tooltip_2 = '概率(客胜): 0.0'
            tooltip_u = '概率(小球 2.5): 0.0'
            tooltip_o = '概率(大球 2.5): 0.0'
        elif target_type == TargetType.RESULT:
            tooltip_1 = f'概率(主胜): {round(self._prob_percentiles["1"][1], 2)}'
            tooltip_x = f'概率(平局): {round(self._prob_percentiles["X"][1], 2)}'
            tooltip_2 = f'概率(客胜): {round(self._prob_percentiles["2"][1], 2)}'
            tooltip_u = '概率(小球 2.5): 0.0'
            tooltip_o = '概率(大球 2.5): 0.0'
        elif target_type == TargetType.OVER_UNDER:
            tooltip_1 = '概率(主胜): 0.0'
            tooltip_x = '概率(平局): 0.0'
            tooltip_2 = '概率(客胜): 0.0'
            tooltip_u = f'概率(小球 2.5): {round(self._prob_percentiles["U"][1], 2)}'
            tooltip_o = f'概率(大球 2.5): {round(self._prob_percentiles["O"][1], 2)}'
        else:
    ...
    def _show_instructions(self):
        QMessageBox.information(
            ...
            '评估说明',
            '请选择要评估的预测目标、模型和数据集。'
            '还可以指定预测时使用的赔率区间和概率百分位过滤器。'
            '这些过滤器不会改变模型输出，只会基于所选比赛对模型进行评估。'
        )
