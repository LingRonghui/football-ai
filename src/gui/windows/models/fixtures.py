...
class FixturesDialog(QDialog):
    ...
    def _add_widgets(self):
        ...
        model_hbox.addWidget(QLabel('赛程日期: '))
        ...
        filters_hbox.addWidget(QLabel('过滤器: '))
    ...
    def _on_date_change(self, qdate: QDate):
        ...
        if df is None:
            QMessageBox.critical(self, '解析失败', '解析赛程失败。请确认所选日期是否正确。')
            scraper.quit()
            return
        ...
        if rows_dropped:
            QMessageBox.information(self, '数据不足', '部分比赛因历史数据不足已被丢弃。')
    ...
    def _read_fixture(self) -> Optional[pd.DataFrame]:
        ...
        for row in range(10):
            ...
            if home == "" and away == "":
                ...
                QMessageBox.critical(self, '缺少主队', f'第 {row} 行缺少主队。')
                return None
            elif away == "":
                QMessageBox.critical(self, '缺少客队', f'第 {row} 行缺少客队。')
                return None
            elif home == away:
                QMessageBox.critical(self, '队伍相同', f'第 {row} 行检测到主客队相同的比赛。')
                return None
            ...
            try:
                ...
                QMessageBox.critical(self, '赔率无效', f'第 {row} 行存在无效或缺失的赔率。')
                return None
            else:
                if odd_1 < 1.01 or odd_x < 1.01 or odd_2 < 1.01:
                    QMessageBox.critical(self, '赔率无效', f'第 {row} 行存在小于 1.01 的赔率。')
                    return None
    ...
    def _export(self):
        ...
        if len(highlight_ids) == 0:
            QMessageBox.information(self, '未选择任何项', '请先选择要导出的比赛（行）。')
            return
        ...
        path, _ = QFileDialog.getSaveFileName(self, '导出为 CSV', default_filepath, 'CSV Files (*.csv)')
