...
class MainWindow(QMainWindow):
    ...
    def __init__(self, app: QApplication):
        ...
        self._qdark_theme_mapper = {'默认': 'auto', '浅色': 'light', '深色': 'dark'}
    ...
    def _add_widgets(self):
        ...
        # --- Analysis Menu ---
        self._menu_analysis = menubar.addMenu('分析')
        self._add_analysis_menus(menu_analysis=self._menu_analysis)

        # --- Train Menu ---
        self._menu_models = menubar.addMenu('模型')
        self._add_model_menus(menu_model=self._menu_models)

        # --- Predict Menu ---
        self._menu_predictions = menubar.addMenu('预测')
    ...
    def _add_file_menus(self, menu_file: QMenu):
        ...
        action_new = QAction('新建联赛', self)
        action_new.setStatusTip('下载并创建一个新联赛')
        action_new.setShortcut(QKeySequence('Ctrl+n'))
        action_new.triggered.connect(new_league)

        self._action_load = QAction('加载联赛', self)
        self._action_load.setStatusTip('加载已有联赛')
        ...
        action_delete = QAction('删除联赛', self)
        action_delete.setStatusTip('从数据库中删除已有联赛')
        action_delete.setShortcut(QKeySequence('Ctrl+d'))
        action_delete.triggered.connect(delete_league)

        self._action_close = QAction('关闭联赛', self)
        self._action_close.setEnabled(False)
        self._action_close.setStatusTip('关闭当前联赛并清空联赛表格')
    ...
    def _add_analysis_menus(self, menu_analysis: QMenu):
        ...
        action_descriptions = QAction('描述统计', self)
        action_descriptions.setStatusTip('分析特征的统计属性')
        action_descriptions.triggered.connect(open_descriptive)

        action_distributions = QAction('数据分布', self)
        action_distributions.setStatusTip('分析特征的分布情况')
        action_distributions.triggered.connect(open_distributions)

        action_variances = QAction('方差分析', self)
        action_variances.setStatusTip('分析每个特征的方差')
        action_variances.triggered.connect(open_variances)

        action_correlation = QAction('相关性分析', self)
        action_correlation.setStatusTip('分析特征之间的相关性')
        action_correlation.triggered.connect(open_correlations)

        action_boruta = QAction('Boruta 特征选择', self)
        action_boruta.setStatusTip('使用 Boruta 分析每个特征的重要性')
        action_boruta.triggered.connect(open_boruta)

        action_coefficients = QAction('系数分析', self)
        action_coefficients.setStatusTip('使用线性回归分析每个特征的重要性')
        action_coefficients.triggered.connect(open_coefficients)

        action_impurity = QAction('不纯度分析', self)
        action_impurity.setStatusTip('使用不纯度分析每个特征的重要性')
        action_impurity.triggered.connect(open_impurity)

        action_rules = QAction('规则提取', self)
        action_rules.setStatusTip('提取可用于解释比赛结果的规则')
    ...
    def _add_model_menus(self, menu_model: QMenu):
        ...
        menu_train = QMenu('训练', self)
        menu_train.setStatusTip('训练机器学习模型以预测比赛结果')
        ...
        action_eval = QAction('评估', self)
        action_eval.setStatusTip('在选定的验证比赛中评估模型')
        action_eval.triggered.connect(lambda _: models.EvaluatorDialog(df=self._league_df, model_db=self._model_db).exec())

        # Creating evaluation menus.
        action_model_manager = QAction('模型管理', self)
        action_model_manager.setStatusTip('删除当前联赛的模型。')
        ...
        menu_interpret = QMenu('可解释性', self)
        menu_interpret.setStatusTip('分析与解释模型的预测结果。')

        action_analysis_logistic = QAction('Logistic Regression', self)
        action_analysis_logistic.setStatusTip('解释/解读 Logistic Regression 模型。')
        ...
        action_analysis_lda.setStatusTip('解释/解读判别分析模型。')
        ...
        action_analysis_tree.setStatusTip('解释/解读 Decision Tree 模型。')
        ...
        action_analysis_forest.setStatusTip('解释/解读 Random Forest 模型。')
        ...
        action_analysis_xgboost.setStatusTip('解释/解读 XGBoost 模型。')
        ...
        action_analysis_knn.setStatusTip('解释/解读 KNN 模型。')
        ...
        action_analysis_nb.setStatusTip('解释/解读 Naive Bayes 模型。')
        ...
        action_analysis_svm.setStatusTip('解释/解读 SVM 模型。')
        ...
        action_analysis_dnn.setStatusTip('解释/解读神经网络模型。')
    ...
    def _add_view_menus(self, menu_view: QMenu):
        ...
        def apply_qdark_theme(theme_name: str):
            ...
            self.statusBar().showMessage(f'主题：{theme_name}')

        # Creating theme menus.
        menu_theme = QMenu('主题', self)
    ...
    def _add_predict_menus(self, menu_predictions: QMenu):
        ...
        action_offline = QAction('手动预测', self)
        action_offline.setStatusTip('手动预测某场比赛的结果')
        action_offline.triggered.connect(lambda _: models.PredictorDialog(df=self._league_df, model_db=self._model_db).exec())

        action_fixtures = QAction('预测赛程', self)
        action_fixtures.setStatusTip('下载并预测整个赛程')
    ...
    def _add_help_menus(self, menu_help: QMenu):
        ...
        lessons_menu = QMenu('学习', self)

        # Adding tutorial menus.
        menu_ml = QMenu('机器学习', self)
        action_ml = QAction('机器学习 (ML)', self)
        action_ml.triggered.connect(lambda _: open_url(url='https://www.ibm.com/think/topics/machine-learning'))

        action_ml_stats = QAction('机器学习与统计学的区别', self)
        ...
        action_supervised = QAction('监督学习', self)
        ...
        action_classification = QAction('分类', self)
        ...
        action_metrics = QAction('分类评估指标', self)
        ...
        menu_algorithms = QMenu('机器学习模型 / 算法', self)
        ...
        action_normalization = QAction('特征归一化', self)
        ...
        action_dnn_improvements = QAction('DNN 改进技巧', self)
        ...
        action_interpretability = QAction('模型可解释性', self)
        ...
        menu_data = QMenu('数据分析', self)
        action_distribution = QAction('变量分布', self)
        ...
        action_correlation = QAction('特征相关性', self)
        ...
        action_boruta = QAction('Boruta 算法', self)
        ...
        menu_advanced = QMenu('进阶机器学习主题', self)
        action_imbalances = QAction('类别不平衡', self)
        ...
        action_smote = QAction('SMOTE 算法', self)
        ...
        action_donation = QAction('请作者喝杯咖啡', self)
