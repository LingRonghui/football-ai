...
class FindDialog(QDialog):
    ...
    def _add_widgets(self):
        ...
        r2 = QHBoxLayout()
        self._label_status = QLabel('0 个匹配')
    ...
    def _find(self):
        ...
        if selected_scope_index == 0 and not exact_match:
            proceed = QMessageBox.warning(
                self,
                '耗时操作',
                '在整个表格中进行非完全匹配搜索是一项非常耗时的操作。是否继续？',
    ...
    def _update_status(self):
        ...
        if n == 0:
            self._label_status.setText('0 个匹配')
            self._btn_prev.setEnabled(False)
            self._btn_next.setEnabled(False)
        else:
            self._label_status.setText(f'{self._match_index + 1} / {n} 个匹配')
...
class ExcelTable(QTableWidget):
    ...
    def _menu(self, pos):
        ...
        m = QMenu(self)
        action_copy = QAction('复制', self, triggered=self.copy_selection)
        action_copy.setToolTip('复制选中的元素。')
        m.addAction(action_copy)

        if self._supports_query_search:
            action_find = QAction('查找', self, triggered=self.open_find_dialog)
            action_find.setToolTip('在表格/列中搜索特定值。')
            m.addAction(action_find)

        hh = self.horizontalHeader()
        is_auto = hh.sectionResizeMode(0) == QHeaderView.ResizeMode.ResizeToContents
        action_resize = QAction('自动调整列宽', self)
        ...
        action_resize.setToolTip('是否启用表头列宽自动调整。可能会影响性能。')
        m.addSeparator()
    ...
    def copy_selection(self):
        ...
        if not ranges:
            if isinstance(self._parent, QMainWindow):
                self._parent.statusBar().showMessage('未复制任何文本。', 3000)
            return
        ...
        if isinstance(self._parent, QMainWindow):
            self._parent.statusBar().showMessage('已复制到剪贴板。', 3000)
...
class SimpleTableDialog(QDialog):
    def __init__(self, df: pd.DataFrame, parent: Optional[QMainWindow], title: str, readonly: bool = True):
        ...
        self.export_btn.setToolTip('将 DataFrame 导出为 (.csv) 文件。若文件已存在，则追加行。')
        ...
        if not path:
            ...
            if len(split_path) == 1:
                ...
                if filetype == 'csv':
                    ...
                    QMessageBox.critical(self, '导出失败', f'无效的文件类型："{filetype}"')
                    return
