...
class StatisticsEngine:
    ...
    def compute_stats(self, df: pd.DataFrame, stat_columns: List[str]) -> pd.DataFrame:
        ...
        # Compute cross-season Elo ratings (pre-match values, no data leakage).
        # Unlike rolling statistics, Elo must NOT be reset per season.
    ...
    def _compute_elo(df: pd.DataFrame) -> pd.DataFrame:
        """ Computes pre-match Elo ratings for the home (HElo) and away (AElo) teams.

            Standard Elo: start at 1500, K-factor 20, expected score via logistic curve,
            actual score is 1 / 0.5 / 0 for win / draw / loss. Ratings evolve continuously
            across seasons so that promoted/relegated teams keep their strength estimate.
            Matches with missing score lines are skipped for the update.

            NOTE: columns MUST be prefixed with 'H'/'A' (HElo/AElo) — the prediction
            pipeline assigns stats to home/away teams by the first character of the
            column name (see inputs.construct_inputs_by_teams).
        """

        elo_ratings = {}
        elo_h, elo_a = [], []
        for home, away, hg, ag in zip(df['Home'], df['Away'], df['HG'], df['AG']):
            rating_home = elo_ratings.get(home, 1500.0)
            rating_away = elo_ratings.get(away, 1500.0)
            elo_h.append(round(rating_home, 1))
            elo_a.append(round(rating_away, 1))

            if pd.isna(hg) or pd.isna(ag):
                continue
            expected_home = 1.0 / (1.0 + 10.0 ** ((rating_away - rating_home) / 400.0))
            actual_home = 1.0 if hg > ag else (0.0 if hg < ag else 0.5)
            elo_ratings[home] = rating_home + 20.0 * (actual_home - expected_home)
            elo_ratings[away] = rating_away + 20.0 * ((1.0 - actual_home) - (1.0 - expected_home))
